...
export class CoinPool extends Component {
     ...
     */
    spawn(pos: Vec3, target: Node, magnetRadius: number, stack: CoinStackTarget | null = null, fromGround = false, stayPut = false): Node {
        const node = this._pool.pop() ?? this._createInstance();
        node.getComponent(Coin)!.activate(pos, target, magnetRadius, this, this._tuning(), stack, fromGround, stayPut);
        return node;
    }
