...
export class DoorAutoOpen extends Component {
    ...
    @property({ displayName: '정지 프레임', tooltip: '열림 클립에서 "완전히 열린" 상태로 멈출 프레임 번호(0=클립 맨 처음 프레임). 열릴 때는 이 프레임에서 멈추고, 닫힐 때는 여기서부터 0번 프레임까지 역재생한다' })
    stopFrame: number = 13;

    @property({ displayName: '클립 FPS', tooltip: '위 "정지 프레임"을 초 단위 재생 시간으로 환산할 때 쓰는 프레임레이트 — 이 클립을 원본에서 내보낼 때 쓴 프레임레이트와 같아야 정확한 프레임에서 멈춘다 (기본 30)' })
    clipFps: number = 30;
    ...
    activate() {
        ...
        // play()가 speed를 건드릴 가능성까지 배제하기 위해 play() 이후에 강제한다.
        state.speed = 0;
        this._dir = 0;
        this._time = 0;
        state.setTime(0);
