      ...
      _export("TowerAttack", TowerAttack = (_dec = ccclass('TowerAttack'), _dec2 = property({
        ...
        update(dt) {
          ...
          if (this._dir.lengthSqr() > 0.0001) {
            ...
            const localZDeg = targetYawDeg - this._restForwardYawDeg; // 조준 방향(Z)과 기울기(X)를 인스펙터의 Euler 표기와 같은 순서로 합성한다 —
        ...
        _spawnMuzzleFx(x, y, z, fwdX, fwdZ) {
          ...
          const mesh = (_this$muzzleMesh = this.muzzleMesh) != null ? _this$muzzleMesh : this.fxHitMesh;
          const mat = (_this$muzzleMat = this.muzzleMat) != null ? _this$muzzleMat : this.fxHitMat;
          if (!mesh || !mat) return;
          const scene = director.getScene();
          if (!scene) return;
