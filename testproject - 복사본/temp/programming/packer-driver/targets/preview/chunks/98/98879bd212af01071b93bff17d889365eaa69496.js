    ...
    execute: function () {
      ...
      _export("StructureUpgradeManager", StructureUpgradeManager = (_dec = ccclass('StructureUpgradeManager'), _dec2 = property({
        ...
        _hideRevealTargets() {
          ...
          var root = (_this$searchRoot = this.searchRoot) != null ? _this$searchRoot : this.node.scene;
          if (!root) return;

          for (var child of root.children) {
            if (!child.name.startsWith(this.revealNamePrefix)) continue;

            for (var r of child.getComponentsInChildren(MeshRenderer)) {
              r.enabled = false;

              this._hidden.push(r);
            }

            for (var _r of child.getComponentsInChildren(SkinnedMeshRenderer)) {
              _r.enabled = false;

              this._hidden.push(_r);
        ...
        _onAllCompleted() {
          ...
          this._done = true;
          var root = (_this$searchRoot2 = this.searchRoot) != null ? _this$searchRoot2 : this.node.scene;
          ...
          if (spots.length > 0) {
            for (var sp of spots) center.add(sp.pos);
          ...
          var _loop = function _loop(t) {
            ...
            var parent = (_t$parent = t.parent) != null ? _t$parent : root;
            var pos = t.worldPosition.clone();
            var rot = t.worldRotation.clone();
            var scale = t.worldScale.clone();
            steps.push({
              key: _this._sweepKey(pos.x, pos.z, center),
              place: true,
              run: () => {
                if (_this.towerPrefab) {
                  var node = instantiate(_this.towerPrefab);
                  parent.addChild(node);
                  node.setWorldPosition(pos);
                  node.setWorldRotation(rot);
                  node.setWorldScale(scale);

                  _this._enableShadow(node);

                  _this._playAppear(node);
                }
          ...
          var _loop2 = function _loop2(old) {
            var p = old.worldPosition;
            steps.push({
              key: _this._sweepKey(p.x, p.z, center),
              place: false,
              run: () => {
          ...
          for (var old of walls.concat(doors)) {
            _loop2(old);
          }
          ...
          var baseDelay = Math.max(0, this.upgradeSweepDuration) + Math.max(0, this.baseUpgradeDelay);
        ...
        _sweepKey(x, z, center) {
          var worldDeg = Math.atan2(x - center.x, z - center.z) * 180 / Math.PI;
          var d = worldDeg - this.upgradeStartAngleDeg;
        ...
        _runSweep(steps) {
          ...
          var total = this.upgradeSweepDuration;

          for (var st of steps) {
            var delay = total > 0 ? total * (st.key / 360) : 0; // scheduleOnce(_, 0)은 다음 프레임으로 밀리므로 시작점 항목은 그냥 즉시 실행한다.
        ...
        _revealTargets() {
          for (var r of this._hidden) if (r.isValid) r.enabled = true;

          this._hidden.length = 0;

          for (var it of this._revealScales) {
        ...
        _swapGroundTexture() {
          ...
          var tex = this.groundRockTexture;
          if (!tex || !this.revealNamePrefix) return;
          var root = (_this$searchRoot3 = this.searchRoot) != null ? _this$searchRoot3 : this.node.scene;
          if (!root) return;

          var _loop3 = function _loop3(child) {
            if (!child.name.startsWith(_this2.revealNamePrefix)) return 0; // continue

            var apply = () => {
              for (var r of child.getComponentsInChildren(MeshRenderer)) {
                var count = r.sharedMaterials.length || 1;
            ...
            var dur = _this2.groundSwapDuration;
            ...
            tween(child).to(half, {
              ...
              easing: _this2.revealScaleEasing
            }).start();
          },
              _ret;
        ...
        _playAppear(node) {
          ...
          var anim = (_node$getComponent = node.getComponent(Animation)) != null ? _node$getComponent : node.getComponentInChildren(Animation);
          if (!anim) return;
          var clip = (_anim$clips$find = anim.clips.find(c => !!c && c.name === this.appearClipName)) != null ? _anim$clips$find : null;
        ...
        _attachBlocker(wall, center) {
          var ws = wall.worldScale;
          var block = new Node('VirtualWall');
          wall.addChild(block);
          block.setScale(this.wallBlockSize.x / (ws.x || 1), this.wallBlockSize.y / (ws.y || 1), this.wallBlockSize.z / (ws.z || 1));
          var vw = block.addComponent(_crd && VirtualWall === void 0 ? (_reportPossibleCrUseOfVirtualWall({
            error: Error()
        ...
        _upgradeBase() {
          ...
          for (var r of this.baseNode.getComponentsInChildren(MeshRenderer)) r.enabled = false;

          for (var _r2 of this.baseNode.getComponentsInChildren(SkinnedMeshRenderer)) _r2.enabled = false;

          var node = instantiate(this.basePrefab);
          this.baseNode.addChild(node);
          node.setPosition(0, 0, 0);
          node.setRotationFromEuler(0, 0, 0);
          var k = this.baseSizeMatchRatio;
        ...
        _enableShadow(node) {
          ...
          for (var r of node.getComponentsInChildren(MeshRenderer)) {
        ...
        _collectWallRing(root, spots, center, steps) {
          ...
          var prefab = this.wallPrefab;
          if (!prefab || spots.length < 2 || this.wallsPerSegment <= 0) return; // 이 중심에서 본 각도로 정렬하면 인접 순서가 곧 링을 한 바퀴 도는 순서가 된다
          // (노드 이름 순서에 의존하지 않는다).

          var ring = spots.slice().sort((a, b) => Math.atan2(a.pos.x - center.x, a.pos.z - center.z) - Math.atan2(b.pos.x - center.x, b.pos.z - center.z));
          var n = ring.length;
          var div = this.wallsPerSegment + 1;

          var _loop4 = function _loop4() {
            var a = ring[i].pos;
            var b = ring[(i + 1) % n].pos; // 벽은 로컬 +X축이 방어선을 따라 뻗는다(씬 실측: 인접 조각을 잇는 방향각과 노드
            // yaw의 차이가 항상 -90도). 그래서 진행 방향각에서 90도를 빼면 올바른 yaw가 된다.

            var yaw = Math.atan2(b.x - a.x, b.z - a.z) * 180 / Math.PI - 90;
            var rot = Quat.fromEuler(new Quat(), 0, yaw, 0);

            var _loop5 = function _loop5() {
              var pos = Vec3.lerp(new Vec3(), a, b, k / div);
              steps.push({
                key: _this3._sweepKey(pos.x, pos.z, center),
                place: true,
                run: () => {
                  ...
                  _this3._enableShadow(node);

                  _this3._playAppear(node);

                  _this3._attachBlocker(node, center);
                }
              });
            };

            for (var k = 1; k <= _this3.wallsPerSegment; k++) {
              _loop5();
            }
          };
