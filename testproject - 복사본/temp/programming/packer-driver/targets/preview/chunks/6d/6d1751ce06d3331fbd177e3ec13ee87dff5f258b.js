  ...
  return {
    ...
    execute: function () {
      ...
      _export("Monster", Monster = (_dec = ccclass('Monster'), _dec(_class = (_class2 = (_class3 = class Monster extends Component {
        ...
        setPath(waypoints, target, lane, laneWidth, laneTaper) {
          if (lane === void 0) {
            lane = 0;
          }
        ...
        static _offsetPath(src, offset, taper) {
          ...
          for (var i = 0; i < n; i++) {
            ...
            var a = src[i === n - 1 ? i - 1 : i];
            var b = src[i === n - 1 ? i : i + 1];
            var dx = b.x - a.x;
            var dz = b.z - a.z;
        ...
        _restoreFlash() {
          for (var mat of this._flashMaterials) {
        ...
        update(dt) {
          ...
          var dest = this._wpIndex < this._waypoints.length ? this._waypoints[this._wpIndex] : this._target.worldPosition;

          var blockingDoor = this._findBlockingDoor(dest, dt);
        ...
        _findBlockingDoor(dest, dt) {
          ...
          var lookahead = Math.min(dist, Math.max(this.moveSpeed * dt, 0.05) + 0.05);
          var nx = myPos.x + dx / dist * lookahead;
          var nz = myPos.z + dz / dist * lookahead;

          for (var door of (_crd && DoorHealth === void 0 ? (_reportPossibleCrUseOfDoorHealth({
            error: Error()
        ...
        _attackDoor(door, dt) {
          ...
          var myPos = this.node.worldPosition;
          var dPos = door.node.worldPosition;
        ...
        _moveToward(dest, dt) {
          ...
          var nz = myPos.z + dz * inv * this.moveSpeed * dt; // 벽이 하필 몬스터 위에 생겨 갇혔다면, 평소 이동 대신 그 벽의 바깥쪽으로 밀려난다.
          ...
          if ((_crd && VirtualWall === void 0 ? (_reportPossibleCrUseOfVirtualWall({
            ...
            var es = this.moveSpeed * MONSTER_EJECT_SPEED_MULT * dt;
