    ...
    execute: function () {
      ...
      _export("HitEffect", HitEffect = (_dec = ccclass('HitEffect'), _dec(_class = class HitEffect extends Component {
        ...
        init(mesh, mat, dir, params) {
          ...
          var inst = mr.getMaterialInstance(0); // 아틀라스(2x2, 4칸) 중 랜덤 1칸만 샘플링. 어떤 그림인지는 넘겨받은 머티리얼이
          ...
          var opacity01 = Math.min(1, Math.max(0, params.opacity * opacityMult)); // mainColor는 unlit 머티리얼이라 그림 색에 그대로 곱해진다 — 색을 주면 그 색으로
          // 물들고, 알파가 세기(오퍼시티)가 된다.

          var tint = params.color;
          ...
          var punchIn = this._life * PUNCH_IN_RATIO;
