      ...
      _export("RushPath", RushPath = (_dec = ccclass('RushPath'), _dec2 = property({
        ...
        _collectAnchors() {
          ...
          for (var child of this.node.children) {
            var m = /^Anchor(\d+)$/.exec(child.name);
            if (m) found.push({
          ...
          return found.map(f => {
            var handleNode = f.node.getChildByName('Handle');
            var handle = handleNode ? Vec3.subtract(new Vec3(), handleNode.worldPosition, f.node.worldPosition) : null;
            return {
        ...
        get waypoints() {
          var anchors = this._collectAnchors();

          if (anchors.length === 0) return [];
          var pts = [anchors[0].pos.clone()];

          for (var i = 0; i < anchors.length - 1; i++) {
            var a = anchors[i],
                b = anchors[i + 1];
            var p1 = a.handle ? Vec3.add(new Vec3(), a.pos, a.handle) : a.pos;
            var p2 = b.handle ? Vec3.subtract(new Vec3(), b.pos, b.handle) : b.pos;

            for (var s = 1; s <= this.samplesPerSegment; s++) {
        ...
        update() {
          ...
          var shouldShow = this.showDebugCurve && anchors.length > 0 && !!this.curveMaterial;
        ...
        _redrawDebug(anchors) {
          ...
          var handleMat = (_this$handleMaterial = this.handleMaterial) != null ? _this$handleMaterial : this.curveMaterial;
          var anchorNodes = gizmo.acquire('anchor', anchors.length, name => new Node(name));
          ...
          var withHandle = anchors.filter(a => a.handle);
          var handleMarkers = gizmo.acquire('handleMarker', withHandle.length, name => new Node(name));
          var handleLines = gizmo.acquire('handleLine', withHandle.length, name => new Node(name));

          for (var _i = 0; _i < withHandle.length; _i++) {
            var a = withHandle[_i];
            var hp = Vec3.add(new Vec3(), a.pos, a.handle);

            this._applyRenderer(handleMarkers[_i], this._getHandleMesh(), handleMat);

            handleMarkers[_i].setWorldPosition(hp);

            this._applyRenderer(handleLines[_i], this._getLineMesh(), handleMat);

            this._placeLine(handleLines[_i], a.pos, hp);
          }

          var pts = this.waypoints;
          var segCount = Math.max(0, pts.length - 1);
          var curveSegs = gizmo.acquire('curveSeg', segCount, name => new Node(name));

          for (var _i2 = 0; _i2 < segCount; _i2++) {
            this._applyRenderer(curveSegs[_i2], this._getLineMesh(), this.curveMaterial);

            this._placeLine(curveSegs[_i2], pts[_i2], pts[_i2 + 1]);
          }
        }
