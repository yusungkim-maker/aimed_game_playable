    ...
    execute: function () {
      ...
      _export("CoinCourierManager", CoinCourierManager = (_dec6 = ccclass('CoinCourierManager'), _dec7 = property({
        ...
        _onSocketFilled(triggerId) {
          ...
          var matches = this.routes.filter(r => r.triggerId === triggerId && r.sourceStack && r.destStack);
          if (matches.length === 0) return;

          this._firedTriggerIds.add(triggerId);

          for (var route of matches) {
            for (var i = 0; i < Math.max(1, route.courierCount); i++) this._spawnCourier(route);
          }
        }
        ...
        _spawnCourier(route) {
          ...
          var node = instantiate(this.courierPrefab); // 출발 무더기 옆에서 등장시킨다 — 소켓 자리와 같은 섹터이므로 플레이어가 소켓을 채운
          ...
          var back = (_node$getComponent = node.getComponent(_crd && CoinStack === void 0 ? (_reportPossibleCrUseOfCoinStack({
            error: Error()
          ...
          back.initialCoins = 0;
          var courier = (_node$getComponent2 = node.getComponent(_crd && CoinCourier === void 0 ? (_reportPossibleCrUseOfCoinCourier({
            error: Error()
          ...
          if (this.weaponNodePrefix) {
            for (var r of node.getComponentsInChildren(SkinnedMeshRenderer)) {
        ...
        _spawnBurst(atWorld) {
          ...
          var e = fx.addComponent(_crd && EndingEffect === void 0 ? (_reportPossibleCrUseOfEndingEffect({
            error: Error()
        ...
        _groundLift(fx) {
          ...
          for (var r of fx.getComponentsInChildren(MeshRenderer)) {
            var _r$mesh;

            var mn = (_r$mesh = r.mesh) == null ? void 0 : _r$mesh.struct.minPosition;
