System.register(["cc", "cc/env"], function (_export, _context) {
  ...
  var _cclegacy, __checkObsolete__, __checkObsoleteInNamespace__, CCObject, Node, EDITOR_NOT_IN_PREVIEW, EditorGizmoController, _crd;

  _export("EditorGizmoController", void 0);
  ...
  return {
    ...
    execute: function () {
      ...
      _cclegacy._RF.push({}, "4ba4eMm7nxMFY/W9pKc3ywm", "EditorGizmoController", undefined);

      __checkObsolete__(['CCObject', 'Node']);
       ...
       *     중복 생성되지 않는다.
       */
      _export("EditorGizmoController", EditorGizmoController = class EditorGizmoController {
        constructor(owner, containerName) {
          this._owner = void 0;
          this._containerName = void 0;
        ...
        acquire(poolName, count, makeNode) {
          var container = this._ensureContainer();

          var pool = this._pools.get(poolName);

          if (!pool) {
            var prefix = poolName + '_';
            pool = container.children.filter(n => n.name.startsWith(prefix)).sort((a, b) => parseInt(a.name.slice(prefix.length), 10) - parseInt(b.name.slice(prefix.length), 10));

            this._pools.set(poolName, pool);
          }

          while (pool.length < count) {
            var node = makeNode(poolName + "_" + pool.length);
            node.hideFlags |= CCObject.Flags.DontSave;
            container.addChild(node);
            pool.push(node);
          }

          for (var i = count; i < pool.length; i++) pool[i].active = false;

          for (var _i = 0; _i < count; _i++) pool[_i].active = true;
        ...
        _ensureContainer() {
          ...
          var c = this._owner.getChildByName(this._containerName);

          if (!c) {
            c = new Node(this._containerName);
          ...
          for (var child of c.children) child.hideFlags |= CCObject.Flags.DontSave;

          c.setPosition(0, 0, 0);
          c.setRotationFromEuler(0, 0, 0);
          c.setScale(1, 1, 1);
          this._container = c; // 컨테이너를 새로 얻은 시점(최초 생성 또는 핫리로드 후 재획득)엔 풀 캐시도
...
//# sourceMappingURL=8949548ddd199e761e6a7a78a30d7da5652aab32.js.map