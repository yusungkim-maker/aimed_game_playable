System.register(["__unresolved_0", "cc", "__unresolved_1", "__unresolved_2", "__unresolved_3"], function (_export, _context) {
  ...
  function seg(name, usedFor, start, duration, priority, speed) {
    if (speed === void 0) {
      speed = 1;
    }

    var s = new SfxSegment();
    s.name = name;
    s.usedFor = usedFor;
    s.start = start;
    s.duration = duration;
    ...
    execute: function () {
      ...
      _export("AudioManager", AudioManager = (_dec9 = ccclass('AudioManager'), _dec10 = property({
        ...
        playCoinGround(coinCount) {
          var per = Math.max(1, this.coinGroundCoinsPerLayer);
          var layers = Math.min(this.coinGroundMaxLayers, Math.max(1, Math.ceil(coinCount / per)));

          for (var i = 0; i < layers; i++) {
            var id = COIN_GROUND_IDS[Math.floor(Math.random() * COIN_GROUND_IDS.length)];
        ...
        playSfx(id, volumeScale, delay) {
          ...
          var startTime = Math.max(0, s.start - this.edgePad);
          var playDuration = s.duration + this.edgePad * 2;

          var i = this._acquireVoice(s.priority);

          if (i < 0) return; // 자기보다 중요한 소리들로 채널이 꽉 참 — 이번 소리는 포기한다

          var src = this._voices[i];
          src.stop();
        ...
        _acquireVoice(priority) {
          ...
          for (var _i = 1; _i < this._voices.length; _i++) {
            if (this._voicePriority[_i] < this._voicePriority[best] || this._voicePriority[_i] === this._voicePriority[best] && this._voiceSeq[_i] < this._voiceSeq[best]) {
              best = _i;
            }
          }
        ...
        _applyRate(src, rate, retries) {
          if (retries === void 0) {
            retries = 4;
          }

          if (rate === 1) return;
          var player = src._player;
          var webNode = player == null ? void 0 : player._sourceNode;
          ...
          var domAudio = player == null ? void 0 : player._domAudio;
        ...
        _setHidden(hidden) {
          ...
          if (hidden) {
            ...
            for (var v of this._voices) if (v.playing) v.stop();
