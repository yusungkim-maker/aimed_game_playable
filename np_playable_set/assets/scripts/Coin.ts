...
export interface CoinTuning {
    ...
    maxWaiting:         number; // 동시에 pop/idle(=아직 수집 안 됨) 상태로 존재할 수 있는 코인 최대 개수
}
...
export class Coin extends Component {
    /** 현재 pop/idle 단계(=아직 수집되지 않고 바닥에 떠 있거나 놓여있는) 상태인 모든 코인을
     * 스폰 순서대로 담아둔다. CoinPool이 여러 개 있어도(플레이어용/각 coin_ground용) 실제
     * "동시에 화면에 떠 있는 미수집 코인 총 개수"는 게임 전체를 통틀어 하나로 제한해야
     * 하므로 인스턴스별이 아니라 static으로 관리한다. */
    private static waiting: Coin[] = [];
    ...
    /** 이 개체가 나르는 자원의 종류. 기본값이 Coin이라, 종류를 넘기지 않는 예전 호출부는
     * 전부 코인으로 취급되어 씬 1·2가 이 필드의 존재를 모르고 지나간다(절대 규칙 5). */
    private _resourceType: ResourceType = ResourceType.Coin;
    get resourceType(): ResourceType { return this._resourceType; }
    ...
    activate(pos: Vec3, target: Node, magnetRadius: number, pool: CoinPool, tuning: CoinTuning, stack: CoinStackTarget | null, fromGround = false, stayPut = false, resourceType: ResourceType = ResourceType.Coin) {
        this._target = target;
        ...
        // stayPut 코인은 대기 목록에 아예 넣지 않는다 — 목록은 "언젠가 회수될 코인"의 큐라서,
    ...
    private _enforceWaitingCap() {
        while (Coin.waiting.length > this._tuning.maxWaiting) {
            const oldest = Coin.waiting[0];
    ...
    private _removeFromWaiting() {
        const i = Coin.waiting.indexOf(this);
    ...
    private _resolveChaseTarget() {
        if (this._stack && this._stackSlot >= 0) {
            this._stack.getSlotWorldPosition(this._stackSlot, this._chaseTarget, this._resourceType);
        } else {
