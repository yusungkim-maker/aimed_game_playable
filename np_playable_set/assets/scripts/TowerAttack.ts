import { _decorator, Component, Node, Vec3, Quat, Animation, AnimationClip, Mesh, Material, director, CCFloat, Color } from 'cc';
...
export class TowerAttack extends Component {
    ...
    @property({ type: CCFloat, displayName: '조준 본 X 기울기(도)', tooltip: '조준할 때 Root_Bow에 적용할 X축 회전. 조준 방향(Z축 회전)은 매 프레임 계산해서 덮어쓰므로, 프리팹에서 본에 직접 넣은 회전값은 남지 않는다 — 대신 이 값이 조준 회전에 함께 합성되어 인스펙터의 본 rotation X로 그대로 나타난다. 타워마다 다르게 두고 싶으면 이 값만 바꾸면 된다' })
    aimPitchDeg: number = 20;
    ...
    arrowSpawnHeight: number = 0.5;

    @property({ displayName: '화살 색 직접 지정', tooltip: '체크하면 아래 색으로 이 타워의 화살만 다르게 칠한다. 끄면 arrowMat 머티리얼 에셋(Arrow01)의 색을 그대로 쓴다 — 머티리얼 에셋 자체는 어떤 경우에도 건드리지 않으므로, 다른 타워나 플레이어 화살에는 영향이 없다' })
    overrideArrowColor: boolean = false;

    @property({
        type: Color,
        displayName: '화살 본체 색',
        visible(this: TowerAttack) { return this.overrideArrowColor; },
    ...
    arrowColor: Color = new Color(255, 255, 255, 255);
    ...
    arrowEmissive: Color = new Color(255, 184, 0, 255);
    ...
    @property({ type: Mesh, displayName: '발사 이펙트 메쉬', tooltip: '비워두면 위의 피격 이펙트 메쉬(fxHitMesh)를 그대로 쓴다 — 보통은 비워두면 된다' })
    muzzleMesh: Mesh | null = null;

    @property({ type: Material, displayName: '발사 이펙트 재질', tooltip: '비워두면 위의 피격 이펙트 재질(fxHitMat)을 그대로 쓴다. 같은 아틀라스(2x2, 4칸)를 쓰며 매 발사마다 4칸 중 하나를 무작위로 고른다' })
    muzzleMat: Material | null = null;

    @property({ displayName: '발사 이펙트 켜기', tooltip: '끄면 발사 순간 번쩍이는 연출을 하지 않는다' })
    muzzleEnabled: boolean = true;

    @property({ type: CCFloat, displayName: '발사 이펙트 앞뒤 위치(m)', tooltip: '번쩍임이 나타나는 지점을 화살이 나가는 자리에서 발사 방향으로 얼마나 밀지. 0이면 화살과 정확히 같은 자리, 양수면 앞(몬스터 쪽), 음수면 뒤(타워 쪽). 좌우/높이는 조절 대상이 아니다 — 높이는 위의 "화살 발사 높이"를 화살과 함께 쓴다' })
    muzzleForwardOffset: number = 0;

    @property({ type: CCFloat, displayName: '발사 이펙트 지속(초)', tooltip: '번쩍임이 나타났다 사라지기까지의 시간. 짧을수록 톡 터지는 느낌' })
    muzzleLifetime: number = 0.18;

    @property({ type: CCFloat, displayName: '발사 이펙트 시작 크기', tooltip: '나타나는 순간의 스케일' })
    muzzleScaleStart: number = 0.15;

    @property({ type: CCFloat, displayName: '발사 이펙트 최대 크기', tooltip: '가장 커졌을 때의 스케일 — 시작 크기에서 여기까지 순간적으로 커진다' })
    muzzleScaleMid: number = 0.8;

    @property({ type: CCFloat, displayName: '발사 이펙트 끝 크기', tooltip: '사라질 때의 스케일 — 최대 크기에서 여기까지 줄어든다' })
    muzzleScaleEnd: number = 0.1;

    @property({ type: CCFloat, displayName: '발사 이펙트 크기 랜덤 최소', tooltip: '위 세 크기 전체에 곱해지는 랜덤 배율의 최소값 — 매 발사마다 조금씩 다른 크기로 터진다' })
    muzzleSizeRandomMin: number = 0.8;

    @property({ type: CCFloat, displayName: '발사 이펙트 크기 랜덤 최대', tooltip: '위 세 크기 전체에 곱해지는 랜덤 배율의 최대값' })
    muzzleSizeRandomMax: number = 1.2;

    @property({ type: CCFloat, displayName: '발사 이펙트 투명도', tooltip: '0~1. 색의 알파로 들어가 번쩍임의 세기가 된다' })
    muzzleOpacity: number = 0.9;

    @property({ type: CCFloat, displayName: '발사 이펙트 투명도 랜덤 최소', tooltip: '투명도에 곱해지는 랜덤 배율의 최소값' })
    muzzleOpacityRandomMin: number = 0.9;

    @property({ type: CCFloat, displayName: '발사 이펙트 투명도 랜덤 최대', tooltip: '투명도에 곱해지는 랜덤 배율의 최대값' })
    muzzleOpacityRandomMax: number = 1.1;
    ...
    private _arrowMatTinted: Material | null = null;
    /** 발사 이펙트에 넘길 방향 벡터 — 발사마다 새로 만들지 않기 위한 공용 임시값 */
    private _muzzleDir = new Vec3();
    ...
    update(dt: number) {
        ...
        if (this._dir.lengthSqr() > 0.0001) {
            ...
            // 조준 방향(Z)과 기울기(X)를 인스펙터의 Euler 표기와 같은 순서로 합성한다 —
    ...
    private _shoot(target: Node) {
        ...
        const spawnFwd = 0.6;
        const spawnHeight = this.arrowSpawnHeight;
        ...
        // 번쩍임은 화살과 같은 자리에서 발사 방향으로만 앞뒤로 밀 수 있다.
        const muzzleFwd = spawnFwd + this.muzzleForwardOffset;
        this._spawnMuzzleFx(
            from.x + fwdX * muzzleFwd,
            from.y + spawnHeight,
            from.z + fwdZ * muzzleFwd,
            fwdX, fwdZ,
        );
        ...
        bNode.addComponent(Bullet).init(
            ...
            this.arrowMesh, this._resolveArrowMaterial(),
    ...
    private _spawnMuzzleFx(x: number, y: number, z: number, fwdX: number, fwdZ: number) {
        if (!this.muzzleEnabled) return;
        const mesh = this.muzzleMesh ?? this.fxHitMesh;
        const mat  = this.muzzleMat  ?? this.fxHitMat;
        if (!mesh || !mat) return;

        const scene = director.getScene();
        if (!scene) return;

        const node = new Node('MuzzleFx');
        scene.addChild(node);
        node.setWorldPosition(x, y, z);
        node.addComponent(HitEffect).init(mesh, mat, this._muzzleDir.set(fwdX, 0, fwdZ), {
    ...
    private _resolveArrowMaterial(): Material | null {
        if (!this.overrideArrowColor || !this.arrowMat) return this.arrowMat;
        if (!this._arrowMatTinted) {
            const m = new Material();
            m.copy(this.arrowMat);
            // 프로퍼티 이름은 builtin-standard 이펙트가 선언한 것 그대로다
            ...
            const s = this.arrowEmissiveScale;
