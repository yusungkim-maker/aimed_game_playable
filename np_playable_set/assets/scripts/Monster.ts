...
import { MonsterDeadEffectSpawner } from './MonsterDeadEffectSpawner';
import { MonsterShadow } from './MonsterShadow';
const { ccclass, property } = _decorator;

/** 벽에 갇힌 몬스터를 밀어내는 속도 배율 — 평소 이동 속도의 몇 배로 빠져나올지.
 * 돌 벽 밴드 두께가 1.5m라 1배면 최대 1초쯤 벽에 박혀 보인다. */
const MONSTER_EJECT_SPEED_MULT = 3;

@ccclass('Monster')
export class Monster extends Component {
    /** ejectDir() 결과를 받는 공용 임시 벡터 (프레임마다 새로 만들지 않기 위함) */
    private static _ejectDir = new Vec3();
    ...
    onLoad() {
        ...
        // 발밑 가짜 그림자 판은 피격 화이트 플래시 대상에서 뺀다. MonsterSpawner가 그림자를
        // 붙인 "뒤에" Monster를 addComponent하므로 이 탐색에 그림자 렌더러까지 걸려서,
        // 몬스터가 맞을 때마다 그림자도 같이 하얗게 번쩍였다. 그림자는 변하지 않는 판이다.
        // 스포너의 호출 순서에 기대지 않고 여기서 명시적으로 거른다(순서가 바뀌어도 안전).
        this._renderers = [
            ...this.getComponentsInChildren(SkinnedMeshRenderer),
            ...this.getComponentsInChildren(MeshRenderer),
        ].filter(r => !Monster._isFakeShadow(r.node, this.node));
    }

    /** node가 가짜 그림자(MonsterShadow) 트리에 속하는지. 프리팹 구조가 바뀌어 판이 한 단계
     *  더 깊어져도 걸리도록 몬스터 루트까지 거슬러 올라간다. 스폰 시 1회만 돈다. */
    private static _isFakeShadow(node: Node | null, stopAt: Node): boolean {
        while (node && node !== stopAt) {
            if (node.getComponent(MonsterShadow)) return true;
            node = node.parent;
        }
        return false;
    }
    ...
    setPath(waypoints: Vec3[], target: Node, lane = 0, laneWidth = 0, laneTaper = 1) {
        this._waypoints = (lane !== 0 && laneWidth > 0)
            ? Monster._offsetPath(waypoints, lane * laneWidth, laneTaper)
            : waypoints;
        this._wpIndex   = 0;
        this._target    = target;
        this._play('move');
    ...
    private static _offsetPath(src: Vec3[], offset: number, taper: number): Vec3[] {
        const n = src.length;
        if (n < 2) return src;
        ...
        for (let i = 0; i < n; i++) {
            ...
            if (len < 0.0001) { out.push(src[i].clone()); continue; }
    ...
    private _die() {
        ...
        // 죽는 순간 그 자리에서 폭발 이펙트가 터진다.
        // 이펙트 크기는 몬스터 스케일(일반 0.5, 보스 1.5 등)을 따라간다.
        MonsterDeadEffectSpawner.instance?.playAt(this.node.worldPosition, this.node.worldScale.y);

        // 몬스터 본체는 dead 애니메이션을 끝까지 재생한 뒤에 사라진다.
        this._curAnim = 'dead';
    ...
    private _moveToward(dest: Vec3, dt: number): boolean {
        ...
        // 벽이 하필 몬스터 위에 생겨 갇혔다면, 평소 이동 대신 그 벽의 바깥쪽으로 밀려난다.
        ...
        if (VirtualWall.ejectDir(myPos.x, myPos.z, Monster._ejectDir)) {
            const es = this.moveSpeed * MONSTER_EJECT_SPEED_MULT * dt;
            this.node.setWorldPosition(
                myPos.x + Monster._ejectDir.x * es,
                myPos.y,
                myPos.z + Monster._ejectDir.z * es,
            );
