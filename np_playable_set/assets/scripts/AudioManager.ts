import { _decorator, Component, Node, AudioClip, AudioSource, CCFloat, CCInteger, input, Input } from 'cc';
...
const ARROW_IDS = [SfxId.Arrow1, SfxId.Arrow2, SfxId.Arrow3];
const COIN_GROUND_IDS = [SfxId.CoinGround1, SfxId.CoinGround2];
const MONSTER_HIT_IDS = [SfxId.MonsterHit1, SfxId.MonsterHit2];

/** 스프라이트 안의 소리 하나 */
@ccclass('SfxSegment')
export class SfxSegment {
    @property({ displayName: '이름' })
    name: string = '';

    @property({ displayName: '발생 위치', tooltip: '이 소리가 어디서 나는지 메모 — 코드 동작에는 영향 없음' })
    usedFor: string = '';

    @property({ type: CCFloat, displayName: '시작(초)' })
    start: number = 0;

    @property({ type: CCFloat, displayName: '길이(초)' })
    duration: number = 0;

    @property({ type: CCFloat, displayName: '볼륨', range: [0, 1], slide: true })
    volume: number = 1;

    @property({ type: CCFloat, displayName: '속도', tooltip: '재생 속도 배율. 2면 길이가 절반으로 압축된다(자르는 게 아니라 빨리 감기라 뒷부분이 잘리지 않는 대신 음정이 그만큼 올라간다). 1이면 원본 그대로' })
    speed: number = 1;

    @property({ type: CCInteger, displayName: '우선순위', tooltip: '숫자가 클수록 중요한 소리. 채널이 모자라면 낮은 쪽을 밀어내고 자리를 차지하고, 반대로 자기보다 높은 소리들만 재생 중이면 이번 소리는 포기한다(중요한 소리를 덮지 않기 위해)' })
    priority: number = 0;
}

function seg(name: string, usedFor: string, start: number, duration: number, priority: number, speed = 1): SfxSegment {
...
export class AudioManager extends Component {
    ...
    // ── BGM ──────────────────────────────────────────────────────────────
    @property({ type: AudioClip, displayName: 'BGM 클립', tooltip: 'audio/bgm/bgm.mp3 — 씬 시작과 함께 루프 재생된다' })
    bgmClip: AudioClip | null = null;

    @property({ type: CCFloat, displayName: 'BGM 볼륨', range: [0, 1], slide: true })
    bgmVolume: number = 0.4;

    // ── SFX ──────────────────────────────────────────────────────────────
    @property({ type: AudioClip, displayName: 'SFX 스프라이트 클립', tooltip: 'audio/sfx/sfx_sprite.mp3 — 9개 효과음이 이어붙어 있는 한 파일' })
    sfxClip: AudioClip | null = null;

    @property({ type: CCFloat, displayName: 'SFX 전체 볼륨', range: [0, 1], slide: true })
    sfxVolume: number = 1;

    @property({ type: [SfxSegment], displayName: '효과음 목록', tooltip: '스프라이트에 담긴 순서 그대로 9개. 각 소리의 시작/길이를 여기서 조정한다' })
    segments: SfxSegment[] = [
        seg('arrow1', '화살/석궁 발사 (랜덤 3종)', 0.000, 0.746, 0, 2),
        seg('arrow2', '화살/석궁 발사 (랜덤 3종)', 0.892, 0.895, 0, 2),
        seg('arrow3', '화살/석궁 발사 (랜덤 3종)', 1.933, 0.775, 0, 2),
        seg('building', '건물 완성', 2.854, 0.959, 3),
        seg('coin', '바닥 코인 획득', 3.960, 0.253, 1),
        seg('coin_ground1', 'coin_ground 수거 (랜덤 2종, 다중 레이어)', 4.359, 0.470, 2),
        seg('coin_ground2', 'coin_ground 수거 (랜덤 2종, 다중 레이어)', 4.975, 0.777, 2),
        seg('monster_hit1', '몬스터 사망 (랜덤 2종)', 5.898, 0.275, 0),
        seg('monster_hit2', '몬스터 사망 (랜덤 2종)', 6.319, 0.377, 0),
    ];

    @property({ type: CCFloat, displayName: '구간 여유(초)', tooltip: '각 구간의 앞뒤로 이만큼 더 넓게 잡아 재생한다. 소리 사이 간격이 0.146초라 이 값이 그 절반(0.073)을 넘지 않으면 옆 소리가 섞이지 않는다 — 소리 앞이 잘린 느낌이면 올리고, 옆 소리가 들리면 내린다' })
    edgePad: number = 0.02;

    @property({ type: CCInteger, displayName: '동시 재생 채널 수', tooltip: '이 개수만큼 AudioSource를 만들어 돌려 쓴다. 모자라면 우선순위가 낮은 소리부터 밀려난다' })
    voiceCount: number = 5;

    // ── coin_ground 다중 레이어 ───────────────────────────────────────────
    @property({ type: CCInteger, displayName: 'coin_ground 최대 레이어', tooltip: '코인이 아무리 많아도 이 개수를 넘겨 겹쳐 재생하지 않는다' })
    coinGroundMaxLayers: number = 5;

    @property({ type: CCInteger, displayName: 'coin_ground 레이어당 코인 수', tooltip: '코인 이 개수마다 레이어가 하나씩 늘어난다 (기본 4 → 1~4개:1겹, 5~8개:2겹 … 17개 이상:5겹)' })
    coinGroundCoinsPerLayer: number = 4;

    @property({ type: CCFloat, displayName: 'coin_ground 레이어 간격(초)', tooltip: '겹쳐 재생할 때 레이어마다 살짝 어긋나게 띄우는 시간 — 0이면 완전히 동시에' })
    coinGroundLayerDelay: number = 0.04;

    private _voices: AudioSource[] = [];
    private _voiceTokens: number[] = [];
    /** 채널별로 지금 재생 중인 소리의 우선순위 */
    private _voicePriority: number[] = [];
    /** 채널별 재생 종료 예정 시각(_now 기준) — 이 시각이 지나면 빈 채널로 본다 */
    private _voiceEnd: number[] = [];
    /** 채널별 재생 시작 순번 — 같은 우선순위끼리는 먼저 시작한 쪽을 먼저 밀어낸다 */
    private _voiceSeq: number[] = [];
    private _tokenSeq = 0;
    private _now = 0;
    ...
    private _onSocketFilled = (triggerId: TriggerId) => {
        ...
        if (triggerId >= TriggerId.GameStart) return;
        this.playSfx(SfxId.Building);
    };
    private _onMonsterKilled = () => this.playRandom(MONSTER_HIT_IDS);
    private _onCoinCollected = (coin: Coin) => {
        // coin_ground(타워가 잡은 코인이 쌓이는 무더기)로 날아간 코인은 소리를 내지 않는다.
        if (coin.stack instanceof CoinGroundStack) return;
        // coin_ground에서 플레이어에게 되돌아오는 코인도 여기(플레이어 스택 도착)로 들어오지만,
        // 그건 수거 시작 시점에 coin_ground 소리를 이미 한 번 냈으므로 개별 소리는 생략한다.
        if (coin.fromGround) return;
        this.playSfx(SfxId.Coin);
    };
    private _onGroundCollect = (coinCount: number) => this.playCoinGround(coinCount);
    /** 등 뒤 스택의 코인이 소켓으로 한 개 빨려들어갈 때마다 — 획득음과 같은 소리를 쓴다 */
    private _onSocketAbsorbed = () => this.playSfx(SfxId.Coin);
    ...
    onLoad() {
        ...
        CoinEvents.on(CoinEventName.SocketFilled, this._onSocketFilled);
        ...
        CoinEvents.on(CoinEventName.GroundStackCollectStarted, this._onGroundCollect);
        CoinEvents.on(CoinEventName.SocketAbsorbed, this._onSocketAbsorbed);
    }

    onDestroy() {
        if (AudioManager.instance === this) AudioManager.instance = null;
        CoinEvents.off(CoinEventName.SocketFilled, this._onSocketFilled);
        ...
        CoinEvents.off(CoinEventName.GroundStackCollectStarted, this._onGroundCollect);
        CoinEvents.off(CoinEventName.SocketAbsorbed, this._onSocketAbsorbed);
    ...
    playSfx(id: SfxId, volumeScale = 1, delay = 0) {
        ...
        if (!this.sfxClip || !s || s.duration <= 0) return;

        // 시작점이 아주 조금만 늦어도 소리의 앞부분(어택)이 잘려 들리므로 앞뒤로 여유를 준다.
        ...
        src.volume = this.sfxVolume * s.volume * volumeScale;
        // 재생 전후로 모두 지정한다 — play()가 재생 위치를 처음으로 되돌리는 구현이 있어서,
        ...
        this._applyRate(src, speed);

        // 구간 길이만큼만 재생하고 멈춘다(배속이면 그만큼 빨리 끝난다). 이 채널이 그 사이
    ...
    private _acquireVoice(priority: number): number {
        ...
        for (let i = 1; i < this._voices.length; i++) {
            if (this._voicePriority[i] < this._voicePriority[best]
                || (this._voicePriority[i] === this._voicePriority[best] && this._voiceSeq[i] < this._voiceSeq[best])) {
                best = i;
            }
        }
    ...
    private _applyRate(src: AudioSource, rate: number, retries = 4) {
        if (rate === 1) return;

        const player = (src as unknown as { _player?: Record<string, any> })._player;
        const webNode = player?._sourceNode;
        if (webNode?.playbackRate) {
            webNode.playbackRate.value = rate;
            return;
        }
        const domAudio = player?._domAudio;
        if (domAudio) {
    ...
    private _onVisibility = () => this._setHidden(document.hidden === true);
    private _onViewable = (viewable: boolean) => this._setHidden(!viewable);

    private _bindVisibility() {
        if (typeof document !== 'undefined' && document.addEventListener) {
            document.addEventListener('visibilitychange', this._onVisibility);
        }
        const mraid = (globalThis as any).mraid;
        if (!mraid || typeof mraid.addEventListener !== 'function') return;
        const hook = () => {
            mraid.addEventListener('viewableChange', this._onViewable);
            // 지금 이미 안 보이는 상태로 시작했을 수도 있다.
