...
export class MonsterSpawner extends Component {
    ...
    @property({ type: Prefab, displayName: '발밑 그림자 프리팹', tooltip: '몬스터 발밑에 깔아줄 알파 텍스처 판(MonsterShadow.prefab). 동적 그림자를 끈 상태에서도 접지감을 유지하기 위한 대체물이다. 비워두면 아무것도 붙지 않는다(기존 동작). 몬스터 프리팹은 glb 자동 생성 서브에셋이라 에디터에서 자식을 추가할 수 없어서 스폰 시점에 코드로 붙인다. 높이·크기 조정은 이 프리팹의 MonsterShadow 컴포넌트에서 한다' })
    fakeShadowPrefab: Prefab | null = null;
    ...
    private _spawnMonster(g: RushGroup, rush: RushConfig) {
        ...
        // 발밑 가짜 그림자 — 판의 높이·크기 조정은 프리팹의 MonsterShadow 컴포넌트가 담당하므로
        // 여기서는 "붙이는 것"만 한다. 위의 monsterCastShadow 루프보다 반드시 "뒤에" 붙여야 한다
        // (그 루프는 자식 렌더러를 전부 캐스터로 만들기 때문). 순서에 의존하지 않도록 이 판의
        // 렌더러는 명시적으로 OFF로 못박는다 — 가짜 그림자가 진짜 그림자를 드리우면 이중으로 보인다.
        if (this.fakeShadowPrefab) {
            const sh = instantiate(this.fakeShadowPrefab);
            for (const r of sh.getComponentsInChildren(MeshRenderer)) {
                r.shadowCastingMode = MeshRenderer.ShadowCastingMode.OFF;
            }
            // addChild 시점에 MonsterShadow.onLoad가 돌면서 부모(몬스터)의 월드 스케일을 읽어
            // 높이를 보정한다. 그래서 monster.setScale 뒤에 붙이는 순서가 중요하다.
            //
            // ⚠ 몬스터 루트의 SkeletalAnimation(baked)이 start()에서 직계 자식 전부의 트랜스폼
            // 갱신을 꺼버려 이 판이 스폰 지점에 얼어붙는다 — 되돌리는 책임은 MonsterShadow.update()에
            // 있다(사유는 그쪽 주석). 여기서 붙이는 순서를 바꿔서 풀 수 있는 문제가 아니다.
            monster.addChild(sh);
        }
