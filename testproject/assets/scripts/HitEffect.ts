...
export class HitEffect extends Component {
    ...
    init(mesh: Mesh, mat: Material, dir: Vec3, params: HitEffectParams) {
        ...
        inst?.setProperty('mainColor', new Color(255, 255, 255, Math.round(opacity01 * 255)));
        ...
        tween(this.node)
            .to(0.08, { scale: new Vec3(s(params.scaleMid), s(params.scaleMid), s(params.scaleMid)) })
            .to(LIFETIME - 0.08, { scale: new Vec3(s(params.scaleEnd), s(params.scaleEnd), s(params.scaleEnd)) })
            .start();
    }
