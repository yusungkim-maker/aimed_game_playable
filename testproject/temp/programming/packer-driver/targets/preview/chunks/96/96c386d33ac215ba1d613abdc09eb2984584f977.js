    ...
    execute: function () {
      ...
      _export("CoinGroundStack", CoinGroundStack = (_dec = ccclass('CoinGroundStack'), _dec2 = property({
        ...
        update(dt) {
          ...
          if (this._stack.length === 0 && this._overflowCount === 0) return;
          if (Vec3.distance(this.node.worldPosition, this.playerNode.worldPosition) > this.radius) return;
        ...
        getSlotWorldPosition(index, out) {
          var lane = index % 4;
          var height = Math.min(Math.floor(index / 4), this.maxPerLane - 1);
          var off = this._laneOffsets[lane];
        ...
        _addOne() {
          ...
          var node = instantiate(this.stackCoinPrefab); // coin_ground 자신이 레벨 배치상 스케일/회전이 걸려있을 수 있어(예: 0.5배 축소, 45도
        ...
        /** 쌓여있던 코인(또는 비주얼 상한을 넘겨 세어만 두었던 초과분) 하나를 등 뒤 스택으로
         * 돌려보낸다. 몬스터를 잡았을 때와 완전히 같은 CoinPool.spawn() 경로(팝 → 자석 추적)를
        ...
        _transferOne() {
          ...
          this.coinPool.spawn(pos, this.playerNode, this.transferMagnetRadius, this._playerStack);
        }
...
//# sourceMappingURL=96c386d33ac215ba1d613abdc09eb2984584f977.js.map