  ...
  return {
    ...
    execute: function () {
      ...
      _export("BuildingObstacle", BuildingObstacle = (_dec = ccclass('BuildingObstacle'), _dec2 = property({
        ...
        onDisable() {
          const i = BuildingObstacle.all.indexOf(this);
        ...
        static isBlocked(x, z) {
          for (const o of BuildingObstacle.all) {
        ...
        _redrawDebug() {
          ...
          const cos = Math.cos(angle),
                sin = Math.sin(angle);
          const hw = this.width * Math.abs(s.x) / 2 + this.margin;
          const hd = this.depth * Math.abs(s.z) / 2 + this.margin;
          const local = [[-hw, -hd], [hw, -hd], [hw, hd], [-hw, hd]];
          const world = local.map(([lx, lz]) => ({
            x: p.x + lx * cos - lz * sin,
            z: p.z + lx * sin + lz * cos
          }));
